...
var Genetic = Genetic || (function(){
	...
	function Genetic() {
		...
		this.start = async function() {
			...
			for (i=0;i<this.configuration.iterations;++i) {
				...
				async function somePromiseFn(ent, thread, socket){
					var f = await self.fitness(ent, thread, socket);
					pop.push({"fitness": f, "entity": ent });
				}

				await (async() => {
				 const p = await Promise.all(this.entities.map(async(e, thread) => await somePromiseFn(e, thread, socket)));
				})();
	...
	Genetic.prototype.evolve = function(config, userData) {
		...
		blobScript += "onmessage = async function(e) { await genetic.start(); }\n";
